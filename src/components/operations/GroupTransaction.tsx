...
export default function GroupTransaction(): JSX.Element {
  ...
  const onSubmitGroupTxns = async (
    ...
    try {
      ...
      const signedTxns = await connection.signTransaction(
        txArr.map((txn) => txn.toByte())
      );
      ...
      console.error(err);
      const temp = err as Error;
      setResponse(temp.message);
    }
  };
      ...
      <div>
        ...
        <TabContent activeTab={activeTab}>
            ...
            <Row className="mt-3">
              <Col xs="12" lg="6" className="mt-2">
                <Form id="payment-tx" onSubmit={onSubmitGroupTxns}>
                  ...
                  <Address
                    label="To for Transaction 1"
                    onChangeAddress={setReceiver1}
                  />
                  <Amount
                    amount={amount1}
                    label="Amount for Transaction 1"
                    onChangeAmount={setAmount1}
                  />
                  <Address
                    label="To for Transaction 2"
                    onChangeAddress={setReceiver2}
                  />
                  <Amount
                    amount={amount2}
                    label="Amount for Transaction 2"
                    onChangeAmount={setAmount2}
                  />
              ...
              <Col xs="12" lg="6" className="mt-2 mt-xs-2">
                <Label className="tx-label">Response</Label>
          ...
          <TabPane tabId="2">
            <Row className="mt-3">
              <Col xs="12" lg="6">
                <Label className="tx-label">Using Algosdk (Recommended)</Label>
                <PrismCode code={codeV2} language="js" />
              </Col>
              <Col xs="12" lg="6" className="mt-xs-4">
                <Label className="tx-label">Another alternative</Label>
                <PrismCode code={codeV1} language="js" />
              </Col>
            </Row>
          </TabPane>
        </TabContent>
